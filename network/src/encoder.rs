...
impl GameEncoder for SimpleGameEncoder {
    ...
    fn encode_move(&self, m: &Move, moving_color: Color) -> u32 {
        ...
        return if let Some(index) = SimpleGameEncoder::REPRESENTATION.iter()
    ...
    fn encode(&self, game: &Game, board_encoder: &impl BoardEncoder) -> Vec<PositionChunk> {
        ...
        for san_move in game.moves.iter() {
            ...
            policy_chunk.set_times_played(1);
            ...
            position_chunk.set_games(1);

            match game.result {
                Some(outcome) => position_chunk.set_points(convert_outcome(outcome)),
                _ => {}
            };
