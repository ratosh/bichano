...
use shakmaty::{Chess, Color, Move, Position, Setup};
use shakmaty::fen::Fen;

use proto::protos::training_chunk::{GameChunk_BoardEncodingType, GameChunk_PolicyEncodingType, PolicyChunk, PositionChunk, GameChunk};
...
pub trait BoardEncoder {
    fn encoding_type(&self) -> GameChunk_BoardEncodingType;
...
pub trait PolicyEncoder {
    fn encoding_type(&self) -> GameChunk_PolicyEncodingType;
    fn encode_move(&self, m: &Move, moving_color: Color) -> u32;
}

pub struct SimplePolicyEncoder {

}

impl SimplePolicyEncoder {
    const REPRESENTATION: [&'static str; 1858] = [
...
impl Default for SimplePolicyEncoder {
    fn default() -> Self {
        Self {}
    }
}
...
impl PolicyEncoder for SimplePolicyEncoder {
    fn encoding_type(&self) -> GameChunk_PolicyEncodingType {
        GameChunk_PolicyEncodingType::SIMPLE_POLICY
    }

    // Simple move encoding (1858 positions):
    fn encode_move(&self, m: &Move, moving_color: Color) -> u32 {
        ...
        let mut representation = if moving_color == Color::White {
            format!("{}{}", from, to)
        } else {
            format!("{}{}", from.flip_vertical(), to.flip_vertical())
        };

        if let Some(promotion_role) = promotion {
            representation.push(promotion_role.char());
        }

        return if let Some(index) = SimplePolicyEncoder::REPRESENTATION.iter()
...
pub fn encode_game<BE: BoardEncoder, PE: PolicyEncoder>(game: &Game, policy_encoder: &PE, board_encoder: &BE) -> GameChunk {
    let mut position = game.position.clone();
    let mut game_chunk = GameChunk::new();
    game_chunk.set_policy_encoding(policy_encoder.encoding_type());
    game_chunk.set_board_encoding(board_encoder.encoding_type());
    ...
    for san_move in game.moves.iter() {
        ...
        println!("Color {}", moving_color.char());

        let mut policies: Vec<PolicyChunk> = Vec::new();
        let mut policy_chunk = PolicyChunk::new();
        policy_chunk.set_weight(1);
        policy_chunk.set_move_index(policy_encoder.encode_move(&parsed_move, moving_color));
